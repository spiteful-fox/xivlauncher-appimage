...
TAG_NAME=""
if [ "${1-''}" = "rb" ]; then
    release_tag_file="latest-rb"
else
    release_tag_file="latest"
fi
TAG_NAME=$(curl -s https://raw.githubusercontent.com/spiteful-fox/xivlauncher-appimage/refs/heads/install-script/$release_tag_file)

APPIMAGE_NAME="$APP_NAME-x86_64.AppImage"
APPIMAGE_URL="https://github.com/spiteful-fox/xivlauncher-appimage/releases/download/$TAG_NAME/$APPIMAGE_NAME"
